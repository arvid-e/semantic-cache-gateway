...
- [ ] 5.1 Implement the gateway authentication middleware
  ...
  - _Depends: 4.2_
- [ ] 5.2 Implement the admin provisioning API with authorization
  - Add admin authorization (constant-time token check) and the routes to create a tenant, issue a gateway key, and attach/rotate/remove a provider credential (plus revoke a key), returning the issued key plaintext only once and never returning stored secret material
  - Observable: requests without valid admin authorization are rejected and make no change; provisioning routes create tenants/keys/credentials; the issue-key response exposes plaintext exactly once; no response returns a stored provider credential or previously issued key
  - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 6.4_
  - _Boundary: Admin Guard, Admin Routes_
  - _Depends: 4.1, 4.2, 4.3, 1.2_
- [ ] 5.3 Register the auth plugin and confirm redaction and endpoint scoping
